...
import 'package:flutter_balcoder_medicalapp/ui/auth/auth_screen.dart';
import 'package:flutter_balcoder_medicalapp/ui/auth/screen/otp/pages/splash_page.dart';
    ...
    await widget.auth.getUser().then((user) {
      setState(() {
        ...
        if (user != null) {
          _userModel = user;
          print("ACA ESTAMOS");
          print(_userModel.userType);

          authStatus = _userModel.uid == null
              ? AuthStatus.NOT_LOGGED_IN
              : AuthStatus.LOGGED_IN;

          if (_userModel.userType == "Usuario") {
            if (_userModel.phoneVerified != null) {
              authStatus = !_userModel.phoneVerified!
                  ? AuthStatus.NOT_PHONE_NUMBER
                  : AuthStatus.LOGGED_IN;
            } else {
              authStatus = AuthStatus.NOT_PHONE_NUMBER;
            }
          } else if (_userModel.userType == "Delivery") {
  ...
  Widget build(BuildContext context) {
    switch (authStatus) {
      case AuthStatus.NOT_DETERMINED:
        ...
        break;
      case AuthStatus.NOT_PHONE_NUMBER:
